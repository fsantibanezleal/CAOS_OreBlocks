...
\newcommand{\versiondoi}{10.5281/zenodo.22834700}
\newcommand{\conceptdoi}{10.5281/zenodo.21512088}
\newcommand{\docrepo}{github.com/fsantibanezleal/CAOS\_OreBlocks}
...
CAOS open-research programme \quad$\cdot$\quad CAOS\_OreBlocks, the \texttt{oreblocks} synthetic block-model package\\[2pt]
...
\thanks{Package \texttt{oreblocks}; source code under the MIT license at
...
{Santib\'a\~nez-Leal: oreblocks, license-free synthetic ore-body block models with a stamped exact ultimate pit}
...
known-by-construction answer without touching licensed data. We validate the solver \emph{independently}:
...
dispatch and haulage tools; every reported number regenerates from the archived validation run.
\end{abstract}
...
redistributed; real mine block models are proprietary. A developer who wants to distribute a reproducible test suite, a
...
a seed and every instance is clearly labelled synthetic. A synthetic benchmark is useful only if its stamped
answer is correct, so we validate the solver \emph{independently} (Section~\ref{sec:validation}) against a
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.78\textwidth]{fig-scaling.pdf}
\caption{Solver validation, from the archived validation run. \textbf{(a)} Independent cross-check: on every
archetype the max-closure pit value agrees with the same instance solved as a linear program by SciPy HiGHS (an
entirely different algorithm) to relative $\sim\!10^{-15}$, machine precision (dashed line). \textbf{(b)} The
exact solve scales sub-second: $0.02$~s at $2{,}048$ blocks to $0.92$~s at $32{,}000$ blocks.}
\label{fig:scaling}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.72\textwidth]{fig-pit.pdf}
...
A synthetic benchmark is only as reliable as its stamped answer, so we check the solver against an independent
method. The ultimate-pit problem also has a linear-programming
...
the identity above), which is at most $2.4\times10^{-7}$, a relative residual of order $10^{-16}$ of the pit
value (double-precision round-off), confirming the internal consistency of every stamped answer. Figure~\ref{fig:pit} shows what an instance looks like: a porphyry cross-section with its exact pit, a
...
tw = make_twin("porphyry", dims=(20,20,10),
               seed=42)
# the exact optimum, stamped
tw.upit.pit_value, tw.upit.n_in_pit
# -> .blocks / .prec / .upit / .meta.json
tw.write("out/")
\end{lstlisting}
\end{minipage}
...
The package consists of small pure-Python modules (\texttt{grid}, \texttt{fields}, \texttt{economics},
...
geology-grounded, license-free instances. Source code and the reproducible validation are available at
\url{https://github.com/fsantibanezleal/CAOS_OreBlocks}.\par}

\section{Limitations}\label{sec:limits}

\texttt{oreblocks} generates instances of the \emph{ultimate-pit} problem (the single-period
...
writes MineLib format precisely so such solvers can consume its instances. The package labels every instance
synthetic and states what its stamped optimum does and does not certify.
...
answer is checked against an independent linear program, with agreement to machine precision.
...
{\raggedright\parindent=1em
The validation is \path{manuscripts/ore-body-twins/figures/upit_validation.py}: it builds each archetype,
...
Figs.~\ref{fig:scaling}--\ref{fig:pit}; the package is seeded and deterministic.\par}
